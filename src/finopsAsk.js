...
function finopsAskCanAccessPanel_(auth, panelId) {
  ...
  if (panelId === 'agreement-dashboard') {
    if (typeof canAccessAgreementDashboard_ === 'function' && !canAccessAgreementDashboard_(auth)) {
      return false;
    }
    if (auth.fiberyAccess === false) {
      return false;
