...
/** @const {number} Per-query result cap (matches agreement PRD Section 4.1). */
...
/* ------------------------------------------------------------------------- */
/* Query builders (agreement PRD Section 4).                                         */
/* ------------------------------------------------------------------------- */
...
/**
 * Section 4.3 Historical (recognized) revenue items.
 * @return {!Object}
 * @private
 */
...
/**
 * Section 4.4 Future (unrecognized, target-date > today) revenue items.
...
function normalizeAgreements_(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    out.push({
      ...
      // for 67%). PRD Section 6/Section 8 thresholds (low margin = 35, bucket cutoffs 35/60)
...
/**
 * Per-agreement counts + Section 5.6 scheduling status, plus per-company agreement
...
/* ------------------------------------------------------------------------- */
/* KPI computation (agreement PRD Section 7.2 + Section 5.2/Section 5.3).                          */
/* ------------------------------------------------------------------------- */
...
function computeKpis_(agreements, companies, companyByName, thresholds) {
  ...
  // Top customer by Total Customer Contract Value (from Companies query, Section 5.4),
...
/**
 * Section 7.2 "Flagged margin" rule: worst negative if any exists; otherwise the
...
/* ------------------------------------------------------------------------- */
/* Chart view models (Section 7.3, Section 7.4, Section 7.9, Section 7.10).                                */
/* ------------------------------------------------------------------------- */
...
/** Section 7.3 Agreement Status Donut. */
...
/** Section 7.10 Agreement Type Mix Donut. */
...
/** Section 7.9 Revenue Recognition Progress (top-N stacked bar; exclude Internal / zero-planned). */
...
/** Section 7.4 Customer Contract Value horizontal bar. */
...
/* ------------------------------------------------------------------------- */
/* Financial Performance Table (Section 7.5).                                        */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* Customer Relationship Cards (Section 7.6)                                         */
/* ------------------------------------------------------------------------- */

/**
 * Builds the Section 7.6 view model: one card per company, sorted by TCV desc with
...
/* ------------------------------------------------------------------------- */
/* Forward Revenue Pipeline (Section 7.8 + Section 5.5)                                     */
/* ------------------------------------------------------------------------- */

/**
 * Builds the Section 7.8 view model. One row per non-Internal agreement, with the
 * Section 5.5 monthly billing rate derived from its future revenue items. Agreements
...
function buildForwardPipeline_(agreements, futureItems, customerColorMap, thresholds) {
  ...
  // months they span (formula Section 5.5).
  var byAgreement = {};
  for (var i = 0; i < futureItems.length; i++) {
    var f = futureItems[i];
    if (!f.agreementId) {
  ...
  for (var j = 0; j < agreements.length; j++) {
    ...
    // Section 7.8: "Each bar represents one agreement with future scheduled revenue
    // items." Agreements with no future items are surfaced separately via the
    // Section 6.5 "no pipeline data" attention alert, so they're skipped here.
...
/* ------------------------------------------------------------------------- */
/* Revenue Flow Sankey (Section 7.11)                                                */
/* ------------------------------------------------------------------------- */

/**
 * Builds the Section 7.11 Sankey view model: three node layers (Status, Customer,
 ...
 *
 * The output matches the contract documented in Section 7.11.7 - `nodes` carries
...
/**
 * Returns the Section 8.5 customer order: non-internal companies sorted by TCV desc.
