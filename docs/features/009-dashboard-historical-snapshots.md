...
> **PRD version 3.20.0** - see `docs/FOS-Dashboard-PRD.md` (**FR-42**, **FR-40**, **FR-104**, **FR-126**, **FR-130**, **FR-137**, **AC-60**, **AC-88**, **AC-92**, **AC-99**; feature **049** bumps agreement schema to **5** with Bid/Program/Initial Planned Hours; **v3.19.1** bumps resource-assignments schema to **5** for Mon–Fri daily Assigned hours; feature **048** had bumped it to **4** with `coverageGaps[]`; **v3.20.0** reuses `fos_viz_range_payloads` for assembled RA payloads with migration **053** fingerprint; feature **047** B6 still ships `personVariancesCodec` without requiring a further bump). Feature **034** reuses the snapshot root for Live daily caches without changing historical snapshot artifacts. Agreement `cacheSchemaVersion` **5**; Delivery projects **2**; Delivery P&L **19** (v3.20.18 hours × cost rate; was **18** for feature **053**; **16** for feature **040** R5 `laborByPerson.allocatedCost`; was **15** for resource alias merge; was **14** for `performance` block; was **13** for laborByPerson logged vs allocated + Allocated & Billable; was **12** for person hours only).
...
|----------|--------|--------|
| `agreement.json` | `buildAgreementDashboardPayload_(snapshotDate)` | Future revenue filtered as of snapshot date; `cacheSchemaVersion: 5` (v3.19.0: Bid/Program/Initial Planned Hours; was **4** in v3.4.4 for `assignedOwner`; was **3**) |
...
| `resource-assignments.json` | `buildResourceAssignmentDashboardPayload_(start, end)` | Fibery Resource Allocations + Labor Costs actuals; range snapshot date **-30 / +90** days; `cacheSchemaVersion: 5` (v3.19.1 Mon–Fri daily Assigned hours; was **4** for v3.18.0 coverageGaps; was **3** for personVariances since v3.7.0; was **2** in v2.19.0). **v3.15.0 (feature 047 B6):** when `PERF_SLIM_RA_PERSON_VARIANCES` is on, `personVariances` is written as positional tuples plus string tables with shared byDay dedup and the artifact carries `personVariancesCodec`. |
| `delivery-pnl/*.json` | `buildDeliveryProjectMonthlyPnLInternal_` | Batched; continuation trigger if needed; `cacheSchemaVersion: 19` (v3.20.18: `laborByPerson.allocatedCost` from hours × user cost rate when Fibery Allocated Cost is empty; was **18** in v3.20.16 / feature **053**: SOW role display + SOW/cost-card planned/projected margin; was **17** in v3.20.14 for `roleOnSow` on assignments; was **16** in v3.7.6 / feature **040** R5: `laborByPerson.allocatedCost`; was **15** in v3.7.3 for resourcesLifetime alias merge; was **14** in v3.6.0 for `performance` block; was **13** in v3.4.12 for laborByPerson allocated hours / % / billable flag; was **12** in v3.4.11 for person hours; was **11** in v3.4.9 for full `resourceAllocations`) |
