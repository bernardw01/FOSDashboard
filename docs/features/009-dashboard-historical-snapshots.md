...
> **PRD version 3.20.0** - see `docs/FOS-Dashboard-PRD.md` (**FR-42**, **FR-40**, **FR-104**, **FR-126**, **FR-130**, **FR-137**, **AC-60**, **AC-88**, **AC-92**, **AC-99**; feature **049** bumps agreement schema to **5** with Bid/Program/Initial Planned Hours; **v3.19.1** bumps resource-assignments schema to **5** for Mon–Fri daily Assigned hours; feature **048** had bumped it to **4** with `coverageGaps[]`; **v3.20.0** reuses `fos_viz_range_payloads` for assembled RA payloads with migration **053** fingerprint; feature **047** B6 still ships `personVariancesCodec` without requiring a further bump). Feature **034** reuses the snapshot root for Live daily caches without changing historical snapshot artifacts. Agreement `cacheSchemaVersion` **5**; Delivery projects **2**; Delivery P&L **16** (feature **040** R5 `laborByPerson.allocatedCost`; was **15** for resource alias merge; was **14** for `performance` block; was **13** for laborByPerson logged vs allocated + Allocated & Billable; was **12** for person hours only).
