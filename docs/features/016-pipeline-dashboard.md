...
- [x] **Access gate (v2.6.1).** Visible when **any** of `Team = CLIENT-ENGAGEMENT`, `Role = EXEC`, or `Role = ADMIN` is true (the v2.6.0 `fiberyAccess` gate was replaced). Enforced by `canAccessPipelineDashboard_()` in both `buildNavigationModel_()` (nav) and `getPipelineDashboardData()` (server returns FORBIDDEN otherwise).
