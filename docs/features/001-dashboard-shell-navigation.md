...
> **PRD version 2.6.10** — see `docs/FOS-Dashboard-PRD.md`.
...
- **Components to create/edit**:
  ...
  - **Favicon** (v2.6.10): harpin brand **`src/assets/favicon.svg`** embedded in **`src/faviconAsset.js`**; `doGet` / not-authorized responses call **`HtmlOutput.setFaviconUrl(getFaviconDataUrl_())`** via `applyWebAppHtmlChrome_` in `Code.js` (not a `<link>` in HTML — Apps Script ignores inline favicon tags). Regenerate with `powershell -File scripts/embed-favicon.ps1`.
