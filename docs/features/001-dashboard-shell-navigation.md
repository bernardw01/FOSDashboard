...
- As any **user**, I want **Profile** and **inactive** dashboard links to respond with an explicit **coming soon** message instead of a broken page.

## Acceptance Criteria (testable)

- [ ] **Given** the Web App is opened as **Execute as: User accessing the web app**, **when** the page loads, **then** the user sees a **left nav** (with **Profile** at the bottom of the nav), **main panel**, and no separate fixed footer bar for Profile.
...
- [ ] **Given** the user clicks **Profile** at the bottom of the left nav, **when** the click completes, **then** the same **coming soon** pattern appears (not a navigation to a new URL).
...
- **Routes/pages impacted**: Single-page shell only (`doGet` → `DashboardShell.html`). No additional Html files for Home/Profile/dashboard bodies in this feature.
- **Components to create/edit**:
  - `src/DashboardShell.html` — layout, nav (sidebar includes Profile), main, Bootstrap modal for “coming soon”, responsive offcanvas sidebar.
...
5. Click each **placeholder dashboard** link: **coming soon** dialog appears and dismisses cleanly.
6. Click **Profile** (bottom of left nav): **coming soon** modal appears.
...
| Phase | What | Outcome |
| --- | --- | --- |
| **1. Shell & static layout** | Add `DashboardShell.html` with semantic regions (`nav`, `main`, sidebar footer for Profile), Bootstrap + theme CSS, and empty states. | Pixel-stable desktop layout. |
| **2. Responsive nav** | Implement sidebar + mobile drawer (toggle, backdrop, `aria-expanded`). | Usable on phone and desktop. |
| **3. Client behaviors** | Home switches main panel content; other nav + Profile open shared **coming soon** Bootstrap **modal**. | Matches “not activating those pages” requirement. |
...
- Real **dashboard modules** (separate HTML partials or client routes) and **Profile** page content.
