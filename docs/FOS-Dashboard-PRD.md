...
**PRD version 1.11.0** — `src/Code.js` constant `FOS_PRD_VERSION` and all `src/*` file headers MUST match the version line below.

Product Requirements Document

Version 1.11.0 - 2026-05-11
...
As of **version 1.11.0**, spreadsheet authorization (FR-05–FR-08a), shell navigation affordances (FR-10a–FR-10b), **whole-app branding contract to `agreement-dashboard-prd-v2.md` §9.5–§9.7 (FR-50)**, **Fibery Agreement Management data contract (`## 6)`)**, **the full Agreement Dashboard with live Fibery data, configurable client-side TTL, attention panel, four Chart.js charts, financial performance table, Customer Relationship Cards (§7.6), Forward Revenue Pipeline (§7.8), and the D3-rendered Revenue Flow Sankey (§7.11)** (`## 7)`, FR-52–FR-56c), and **user activity logging (`## 3.8`, FR-60–FR-66)** are documented; acceptance criteria through **AC-21** are tracked; implementation status follows each **[Released]** / **[Backlog]** tag. The user-facing left-nav label and the internal route id are both **Agreement Dashboard** (`agreement-dashboard`); the prior `finance` route id was retired in v1.11.0 — historical activity-log rows tagged `finance` remain queryable alongside new `agreement-dashboard` rows.
