...
**FOS** (Finance & Operations Snapshot) is a **Google Apps Script** web application that gives authorized Workspace users a single place to open **harpin AI Ops Dashboards**: a shell with navigation, spreadsheet-backed access control, an **Agreement Dashboard** view (Fibery-backed Agreement Management), a **Utilization Management Dashboard** and **Labor hours** week review (Fibery-backed Labor Costs), and a **Delivery Dashboard** (Active Projects table with substring search + per-project monthly P&L with chart / table view, per-month Revenue drill-down, projected-month coverage, Copy CSV, and as of **v1.21.0** a pacing strip, delivery-signals strip, and portfolio margin-flow Sankey; as of **v1.25.0** a **Revenue review** child panel under **Delivery** reuses the Agreement Dashboard browser cache for monthly revenue KPIs; as of **v1.26.0** that panel adds sortable milestone **tables**, **Copy CSV**, **print**, and customer→agreement **milestone drill-down**) — all aligned with the product baseline in `[docs/FOS-Dashboard-PRD.md](docs/FOS-Dashboard-PRD.md)`.
