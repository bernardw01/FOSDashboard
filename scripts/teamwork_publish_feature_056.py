#!/usr/bin/env python3
"""Publish Feature 056 notebook and Spec Draft release task.

Related origin (Jess, Engagement Review inbox):
...
FEATURE_ID = "056"
RELEASE_TITLE = "Monthly Client Financial Performance Review (Lookback)"
TASK_NAME = f"Feature {FEATURE_ID} - {RELEASE_TITLE}"
NOTEBOOK_KEY = "feature_056"
NOTEBOOK_TITLE = "Feature 056 - Monthly Client Financial Performance Review (Lookback)"
NOTEBOOK_DESC = (
    "Locked monthly lookback inside Engagement review. Jess answers 2026-09-08 locked; "
    "remaining R1-R7 (EAC vs actual, executed contracts, narrative boxes, ship slice, "
    "holidays, Guy Y2D)."
)
TASKLIST_NAME = "Delivery"
INBOX_TASK_ID = 40478571
MD_PATH = ROOT / "docs/features/056-monthly-lookback-financial-review.md"
HOW_WE_WORK_KEY = "how_we_work"

SCOPE = """**Scope (draft - pending Jess Q1-Q15 in notebook):**

Follow-on to shipped Engagement Review (037). Do not reuse the 037 Shipped task.

1. **Monthly lock:** After first Sunday time deadline + one business day, capture and freeze the Lookback for that reporting month (live dashboards stay independent).
2. **Auto-select** projects (configurable margin floor, MoM/plan variance, red health, closed-in-month, change orders); PMs cannot remove auto rows.
3. **Manual opt-in** by PM or Finance with Selected By + optional Reason.
4. **Review trigger** on each selected project.
5. **Owner narrative** (five fields or mapped DEAP fields), Narrative Status, screenshot evidence.
6. **Lookback view:** selected-only, Ready vs Action Required, month archive read-only after the cycle.
7. Meeting facilitation reuses 037 calendar/notes/recordings/synopsis where Q1 allows.

**Jess:** please answer Q1-Q15 in the feature notebook before Spec Approved."""
...
def feature_html(md_path: Path) -> str:
    ...
    return markdown_to_notebook_html(md)
...
**Related origin (037 inbox):** {inbox_url}
**Requestor:** Jess Williams
**Source:** Monthly Client Financial Performance Review (Lookback) Workflow PDF
...
def update_feature_md_links(*, notebook_url: str, release_task_url: str) -> None:
    md = MD_PATH.read_text(encoding="utf-8")
    old_nb = "> **Teamwork notebook:** TBD (publish script)  \n"
    old_task = "> **Release task (Spec Draft):** TBD  \n"
    new_nb = f"> **Teamwork notebook:** [{NOTEBOOK_TITLE}]({notebook_url})  \n"
    new_task = f"> **Release task:** [{TASK_NAME}]({release_task_url})  \n"
    if old_nb in md and old_task in md:
        md = md.replace(old_nb, new_nb).replace(old_task, new_task)
        MD_PATH.write_text(md, encoding="utf-8")
        print(f"Updated {MD_PATH.name} Teamwork links")
        return
    if NOTEBOOK_TITLE in md and TASK_NAME in md:
        print(f"{MD_PATH.name} already has Teamwork links")
        return
    print(f"WARNING: could not patch Teamwork links in {MD_PATH.name}")
...
def main() -> None:
    ...
    print(f"Linked related inbox {INBOX_TASK_ID} -> {release_url}")
