#!/usr/bin/env python3
"""Publish Feature 055 notebook and Spec Draft release task.

Inbox source (Jess, 2026-07-17):
https://win.godeap.io/app/tasks/40521091
"""
...
FEATURE_ID = "055"
RELEASE_TITLE = "Planned margin on PM Overview and Portfolio P&L"
TASK_NAME = f"Feature {FEATURE_ID} - {RELEASE_TITLE}"
NOTEBOOK_KEY = "feature_055"
NOTEBOOK_TITLE = "Feature 055 - Planned margin on PM Overview and Portfolio P&L"
NOTEBOOK_DESC = (
    "Show planned margin alongside actual margin on PM Overview (list + project detail) "
    "and Finance Portfolio P&L. Pending Jess answers on definition and Portfolio rollup."
)
TASKLIST_NAME = "Delivery"
INBOX_TASK_ID = 40521091
MD_PATH = ROOT / "docs/features/055-planned-margin-hub-locations.md"
HOW_WE_WORK_KEY = "how_we_work"

SCOPE = """**Scope (draft - pending Jess Q1-Q10 in notebook):**

1. **PM Overview - Active projects list:** Planned margin column next to Margin; rename margin to **Actual** per Jess.
2. **PM Overview - Project detail (Accounting P&L):** Planned margin near Table/Chart/Copy CSV; actual label on margin KPI/grid.
3. **Finance - Portfolio P&L:** Planned summary + **Planned margin $ / %** rows; **Actual** labels on margin rows.

**Open:** Planned margin definition (Target Margin vs SOW-rate **053**), Portfolio rollup formula, Accounting vs Performance overlap.

**Questions for Jess** are in the notebook (Q1-Q10). Do not implement until Spec Approved."""
...
def update_feature_md_links(*, notebook_url: str, release_task_url: str) -> None:
    ...
    old = "> **Teamwork:** Notebook + release task not published yet (run `scripts/teamwork_publish_feature_055.py`)  \n"
    if old in md:
