#!/usr/bin/env python3
"""Publish Feature 051 notebook and Spec Draft release task.

Inbox source (Jess, 2026-08-25):
https://win.godeap.io/app/tasks/40886771
"""
...
FEATURE_ID = "051"
RELEASE_TITLE = "PM Overview contract duration on project select"
TASK_NAME = f"Feature {FEATURE_ID} - {RELEASE_TITLE}"
NOTEBOOK_KEY = "feature_051"
NOTEBOOK_TITLE = "Feature 051 - PM Overview contract duration on project select"
NOTEBOOK_DESC = (
    "Show a labeled Contract duration KPI chip on PM Overview when a project is "
    "selected, using existing Fibery duration fields on the delivery payload."
)
TASKLIST_NAME = "Delivery"
INBOX_TASK_ID = 40886771
MD_PATH = ROOT / "docs/features/051-pm-overview-contract-duration.md"
HOW_WE_WORK_KEY = "how_we_work"

SCOPE = """**Scope:** PM Overview → Project financials card when a project is selected:

1. Add **Contract duration** KPI chip (locale-friendly date range + days remaining/ended subtext).
2. Use existing `durStart` / `durEnd` / `executionDate` on delivery project rows (no new Fibery query).
3. Remove duration from the subtitle line to avoid duplication.
4. Mobile: same chip in the wrapping KPI strip.

**Draft spec** - pending Jess review of locked decisions in the notebook."""
