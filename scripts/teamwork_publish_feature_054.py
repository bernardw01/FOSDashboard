#!/usr/bin/env python3
"""Publish Feature 054 notebook and Spec Draft release task.

Inbox source:
https://win.godeap.io/app/tasks/40507255
"""
...
FEATURE_ID = "054"
RELEASE_TITLE = "Alert emails from shared or group address"
TASK_NAME = f"Feature {FEATURE_ID} - {RELEASE_TITLE}"
NOTEBOOK_KEY = "feature_054"
NOTEBOOK_TITLE = "Feature 054 - Alert emails from shared or group address"
NOTEBOOK_DESC = (
    "Optional NOTIFICATIONS_FROM_EMAIL Settings field sends alert digests and "
    "hydrate-failure emails via Gmail send-as alias instead of the script owner MailApp address."
)
TASKLIST_NAME = "Admin and settings"
INBOX_TASK_ID = 40507255
MD_PATH = ROOT / "docs/features/054-notification-shared-from-address.md"
HOW_WE_WORK_KEY = "how_we_work"

SCOPE = """**Scope:** Feature 033 alert email digests (and hydrate-failure admin emails):

1. New ADMIN Settings property **`NOTIFICATIONS_FROM_EMAIL`** (optional).
2. When set to a verified Gmail **Send mail as** alias on the script owner account, use **`GmailApp.sendEmail`** with that **`from`** address and **`NOTIFICATIONS_FROM_NAME`**.
3. When empty, keep today's **`MailApp`** behavior (script owner address + display name only).
4. Failed alias sends log error; no silent fallback to personal address.
5. Diagnostic **`_diag_notificationFromEmail_()`** lists configured address and Gmail aliases.

**Ops prerequisite:** verify the group/shared address as Send mail as on the Apps Script owner Gmail account before enabling."""
...
def update_feature_md_links(*, notebook_url: str, release_task_url: str) -> None:
    ...
    if "> **Teamwork notebook:**" in md and "TBD" not in md:
        md = md.replace(
            "> **Teamwork notebook:** TBD (publish script)  \n"
            f"> **Release task (Spec Draft):** TBD  \n",
            replacement,
        )
