#!/usr/bin/env python3
"""Publish Feature 053 notebook and Spec Draft release task.

Inbox source (Jess, 2026-08-24):
https://win.godeap.io/app/tasks/40876280
"""
...
FEATURE_ID = "053"
RELEASE_TITLE = "PM Overview SOW role and SOW-based planned margin"
TASK_NAME = f"Feature {FEATURE_ID} - {RELEASE_TITLE}"
NOTEBOOK_KEY = "feature_053"
NOTEBOOK_TITLE = "Feature 053 - PM Overview SOW role and SOW-based planned margin"
NOTEBOOK_DESC = (
    "Replace Role with SOW role on PM Overview Project Performance and compute "
    "Planned margin from SOW bill/cost rates with honest missing-input states."
)
TASKLIST_NAME = "Delivery"
INBOX_TASK_ID = 40876280
MD_PATH = ROOT / "docs/features/053-pm-overview-sow-margin-and-role.md"
HOW_WE_WORK_KEY = "how_we_work"

SCOPE = """**Scope:** PM Overview → Project Performance tab when a project is selected:

1. Rename resource table column **Role** → **SOW role**; show Fibery **Role on SOW** when **Allocated & Billable**; otherwise **-**.
2. Replace **Planned margin** KPI (today: agreement Target Margin) with margin from SOW bill/cost rate inputs when available.
3. Tooltip: margin reflects numbers used when the SOW was written.
4. When bill and cost SOW rates are both missing: Planned margin **-** with a visible missing-input indicator.
5. Mobile: same rules on resource cards and KPI chips.

**Draft spec** - pending Jess review of **Questions for Jess** (Q1-Q7) in the notebook."""
